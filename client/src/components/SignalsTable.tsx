...
export function SignalsTable() {
  const { data: analyses, isLoading, error } = useCodeAnalyses();
  ...
  if (!analyses || analyses.length === 0) {
    return (
      <div className="h-[400px] flex flex-col items-center justify-center text-muted-foreground gap-4">
        ...
        <p>No code analyses yet — run your first analysis above</p>
      </div>
    );
  }
                ...
                <TableCell className="text-right">
                  <div className="flex items-center justify-end gap-2">
                    <div className="w-12 h-1.5 bg-white/10 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${
                          ...
                          score > 0.3 ? 'bg-orange-500' : 'bg-rose-500'
                        }`}
                    ...
                    <span className={`text-xs font-mono font-bold ${
                      ...
                      score > 0.3 ? 'text-orange-400' : 'text-rose-400'
                    }`}>
                      {(score * 100).toFixed(0)}%
                    </span>
                  </div>
